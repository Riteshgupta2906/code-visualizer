...
/**
 * Finds all files that import the specified target file
 */
export function findFilesImporting(targetFilePath, projectRoot) {
  try {
    ...
    files.forEach((file) => {
      ...
      if (path.resolve(file) === normalizedTarget) return;

      const dependencies = analyzeDependencies(file, projectRoot);

      // Check if any local dependency resolves to the target file
      const importsTarget = dependencies.localDependencies.some((dep) => {
        return (
          dep.resolvedPath &&
          path.resolve(dep.resolvedPath) === normalizedTarget
        );
      });
    ...
    console.error("Error finding importing files:", error);
    return [];
  }
}
