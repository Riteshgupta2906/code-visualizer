...
function extractAPIMethodsFromFile(filePath) {
  try {
    ...
    const methodPatterns = [
      ...
      // Arrow function exports (const/let/var)
      /export\s+const\s+GET\s*=\s*(async\s+)?\(/gi,
      /export\s+const\s+POST\s*=\s*(async\s+)?\(/gi,
      /export\s+const\s+PUT\s*=\s*(async\s+)?\(/gi,
      /export\s+const\s+DELETE\s*=\s*(async\s+)?\(/gi,
      /export\s+const\s+PATCH\s*=\s*(async\s+)?\(/gi,
      /export\s+const\s+HEAD\s*=\s*(async\s+)?\(/gi,
      /export\s+const\s+OPTIONS\s*=\s*(async\s+)?\(/gi,
    ];
    ...
    methodPatterns.forEach((pattern, index) => {
      if (pattern.test(fileContent)) {
        // Use modulo to map back to the correct method name
        methods.add(methodNames[index % 7]);
      }
    });
